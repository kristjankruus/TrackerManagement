...
public class WindowController : MonoBehaviour
{
    ...
    private TrackerManager_Director director;
    ...
    private TrackerManagerTrayForm trayForm;
    ...
    void Start()
    {
        director = GetComponent<TrackerManager_Director>();
    ...
    public void CreateTray()
    {
        ...
        if(trayForm == null)
        {
            trayForm = new TrackerManagerTrayForm(trayIconTex); //CreateIcon(trayIconTex));
...
public class TrackerManagerTrayForm : System.Windows.Forms.Form 
{
    ...
    public TrackerManagerTrayForm(Texture2D tex = null)
    {
        trayMenu = new System.Windows.Forms.ContextMenuStrip();
        
        if(tex)
            trayMenu.Items.Add("Show TrackerManagement", CreateBitmap(tex), OnShowWindow);
        else
            trayMenu.Items.Add("Show TrackerManagement", CreateBitmap(Texture2D.whiteTexture), OnShowWindow);

        trayMenu.Items.Add("Exit", CreateBitmap(Texture2D.whiteTexture), OnExit);

        trayIcon = new NotifyIcon();
        trayIcon.Text = "TrackerManagement";
