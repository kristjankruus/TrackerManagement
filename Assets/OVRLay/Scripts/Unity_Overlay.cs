...
public class Unity_Overlay : MonoBehaviour 
{
	...
	void Start () 
	{
		...
		if(Application.isPlaying)
		{
            ...
            if (canvasText != null && customDeviceIndex > 0 || deviceToTrack == OverlayTrackedDevice.RightHand || deviceToTrack == OverlayTrackedDevice.LeftHand)
            {                
                InvokeRepeating("GetBatteryLevel", 2.0f, 10.0f);
            }           
		}
	}
    ...
    void GetBatteryLevel()
    {
        ...
        canvasText.text = string.Format("{0}%", batteryLevel * 100);
    }
